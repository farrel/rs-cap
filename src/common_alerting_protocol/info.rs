...
//
// impl Info {
//     pub fn deserialize_from_xml(namespace: &[u8], reader: &mut Reader<&[u8]>) -> Result<Info, DeserialiseError> {
//         let mut buf = Vec::new();
...
//                     EVENT_TAG => info.event = Some(parse_string(reader, EVENT_TAG)?),
...
//         let info = Info::deserialize_from_xml(VERSION_1_2, reader).unwrap();
//
